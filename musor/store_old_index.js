...
import { compare } from '@/store/helpers';
  ...
  return new Vuex.Store({
    ...
    actions: {
      nuxtServerInit(vuexContext, context) {
        return context.app.$axios.$get('/nuxt-blog.json')
          .then(data => {
            const postArray = [];
      ...
      addPost(vuexContext, postData) {
        ...
        return this.$axios.$post('/nuxt-blog.json?auth=' + vuexContext.state.token, createdPost)
          .then(data => vuexContext.commit('addPost', { ...createdPost, id: data.name }))
          .catch(e => console.log(e))
      },
      editPost(vuexContext, editedPost) {
        return axios.put("https://nuxt-init.firebaseio.com/nuxt-blog/" +
          editedPost.id +
      ...
      initAuth(vuexContext, request) {
        ...
        if (new Date().getTime() > +expirationDate || !token) {
          console.log('No token on invalid token')
    ...
    getters: {
      loadedPosts(state) {
        return state.loadedPosts.sort(compare)
      },
